...
var BASE_REVIEWS = [
  ...
  },
  {
    ...
    text: 'Обратились с кроликом к Ахмаду Барфину Хасану. Врач внимательно осмотрела Пуговку, подробно объяснила, как ухаживать за экзотическим питомцем, и дала понятные рекомендации. Очень спокойно и бережно работает с животными. Спасибо за внимательный подход, обязательно придём на контрольный осмотр.',
    stars: 5,
    avatar: '👩',
    date: '3 мая 2025',
    reply: ''
...
function goPriceTab(tab) {
  showPage('price');
  setTimeout(function() {
    var target = Array.prototype.find.call(document.querySelectorAll('.price-tab'), function(btn) {
      return btn.getAttribute('onclick') && btn.getAttribute('onclick').indexOf("'" + tab + "'") !== -1;
    });
    showPriceTab(tab, { target: target });
  }, 120);
}

document.addEventListener('keydown', function(e) {
  if ((e.key !== 'Enter' && e.key !== ' ') || !e.target.classList || !e.target.classList.contains('feature-card')) return;
  e.preventDefault();
  e.target.click();
});
...
function updateTimeSlots() {
  ...
  var clinicEl = document.getElementById('f-clinic');
  var clinic  = clinicEl ? clinicEl.value : '';
...
var SYMPTOMS = {
  'Собака': ['Вялость / слабость','Отказ от еды','Рвота','Диарея','Кашель','Одышка','Судороги','Хромота','Кровь в моче','Понос с кровью','Резкое похудение','Зуд / расчёсы','Необычное слюнотечение','Неприятный запах из пасти','Частое мочеиспускание','Перхоть','Увеличение живота','Слезотечение','Чихание'],
  'Кошка': ['Вялость / слабость','Отказ от еды','Рвота','Понос','Кашель','Одышка','Судороги','Кровь в моче','Затруднённое мочеиспускание','Частое мочеиспускание','Резкое похудение','Зуд / облысение','Необычное слюнотечение','Неприятный запах из пасти','Перхоть','Увеличение живота','Слезотечение','Чихание','Трудности при дыхании'],
  ...
  'Грызун': ['Вялость','Отказ от еды','Понос','Одышка','Судороги','Опухоли / шишки','Зуд','Облысение','Слюнотечение','Разросшиеся зубы'],
  'Кролик': ['Отказ от еды','Нет кала / мало кала','Вздутие живота','Скрежет зубами','Наклон головы','Выделения из носа','Слюнотечение','Разросшиеся зубы','Понос','Вялость','Одышка','Судороги','Зуд','Облысение'],
  'Рептилия': ['Отказ от корма','Вялость','Необычное дыхание / открытый рот','Выделения из носа или рта','Проблемы с линькой','Опухшие глаза','Мягкий или деформированный панцирь','Вздутие живота','Диарея','Запор','Регургитация','Потеря веса','Нарушение координации','Травма / ожог','Изменение цвета кожи','Клещи на коже','Долго не выходит из укрытия']
};

var URGENT = ['Одышка','Судороги','Понос с кровью','Кровь в моче','Затруднённое мочеиспускание','Трудности при дыхании','Увеличение живота','Нет кала / мало кала','Вздутие живота','Необычное дыхание / открытый рот','Мягкий или деформированный панцирь','Нарушение координации','Травма / ожог'];
var MEDIUM = ['Рвота','Диарея','Понос','Отказ от еды','Хромота','Резкое похудение','Необычное слюнотечение','Неприятный запах из пасти','Частое мочеиспускание','Перхоть','Проблемы с линькой','Регургитация','Потеря веса'];
...
function selectAnimal(animal, el) {
  ...
  var html = syms.map(function(s) {
    return '<span class="symptom-chip" onclick="toggleSymptom(this,\'' + s.replace(/'/g,"\\'") + '\')">' + s + '</span>';
  }).join('');
  html += '<span class="symptom-chip" onclick="openOtherSymptomAppointment()">Другое</span>';
  document.getElementById('symptomGrid').innerHTML = html;
}

function openOtherSymptomAppointment() {
  openModal();
  setTimeout(function() {
    var animalSelect = document.getElementById('f-animal');
    var comment = document.getElementById('f-comment');
    if (animalSelect && checkerAnimal) {
      var animalOptions = {
        'Собака': '🐕 Собака',
        'Кошка': '🐈 Кошка',
        'Кролик': '🐇 Кролик',
        'Грызун': '🐀 Грызун / Крыса',
        'Птица': '🦜 Птица',
        'Рептилия': '🦎 Рептилия / Экзотология'
      };
      animalSelect.value = animalOptions[checkerAnimal] || '';
    }
    if (comment && !comment.value.trim()) comment.value = 'Другой симптом: ';
...
function lbNav(dir) {
  if (!lbFiltered.length) return;
  lbIndex = (lbIndex + dir + lbFiltered.length) % lbFiltered.length;
  renderLb();
}

function getGalleryPhoto(idx) {
  var item = document.querySelectorAll('.gallery-item')[idx];
  if (item) {
    var imgEl = item.querySelector('img');
    var labelEl = item.querySelector('.gi-overlay-label');
    return {
      src: imgEl ? imgEl.getAttribute('src') : '',
      caption: labelEl ? labelEl.textContent.trim() : (imgEl ? imgEl.getAttribute('alt') || '' : '')
    };
  }
  return galleryPhotos[idx] || { src: '', caption: '' };
}

function renderLb() {
  if (!lbFiltered.length) return;
  var photo = getGalleryPhoto(lbFiltered[lbIndex]);
